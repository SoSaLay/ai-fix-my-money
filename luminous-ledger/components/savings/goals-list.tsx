...
import {
  ...
  Folder, FolderOpen, ChevronDown, ChevronRight, Target,
} from 'lucide-react'
  ...
  return (
    <div className="flex flex-col gap-3 bg-surface-container rounded-xl p-4 border border-secondary/20">
      <div className="flex items-center gap-2">
        ...
        <p className="text-label-md font-semibold text-on-surface">New Goal Folder</p>
      </div>
      <p className="text-label-sm text-on-surface-variant">
      ...
      <input
        ...
        placeholder="Folder name (e.g. Home Renovation)"
        autoFocus
...
function FolderSection({
  ...
  const handleDeleteFolder = () => {
    if (!confirm(`Remove folder "${folder.name}"? Goals inside will become standalone goals.`)) return
    ...
    <div className="flex flex-col rounded-xl border border-surface-container-high overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-3 bg-surface-container-low">
        ...
        >
          ...
          {collapsed
            ...
            : <FolderOpen size={15} className="text-secondary shrink-0" />}
      ...
      {!collapsed && (
        <div className="flex flex-col gap-3 px-4 py-3">
          {/* Aggregate progress bar (only when 2+ goals) */}
          {goals.length > 1 && (
...
function AddMenu({
  ...
  return (
    ...
    >
      ...
      >
        <Folder size={16} className="text-secondary mt-0.5 shrink-0" />
        <div>
...
export function GoalsList({
  ...
  // Goals not assigned to any existing folder
