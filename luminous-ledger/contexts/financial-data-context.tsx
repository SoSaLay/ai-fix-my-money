...
export function FinancialDataProvider({ children }: { children: ReactNode }) {
  ...
  // Derive one account per named group from statements (only when no AI data)
  const { assets: stmtAssets, debts: stmtDebts } = !financialData && statements.length > 0
  ...
  const dashboardSummary = financialData
    ? deriveDashboardSummary(financialData, spendingLimit, savingsGoals, investingGoal, generalSavingsPct)
    : deriveSummaryFromStatementMonths(statementMonths, spendingLimit, savingsGoals, investingGoal, generalSavingsPct)
