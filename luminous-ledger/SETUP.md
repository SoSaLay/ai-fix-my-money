# Luminous Ledger — Setup & Self-Hosting Guide
...
## What is Luminous Ledger?

Luminous Ledger is a **local-first, privacy-focused personal finance dashboard** built with Next.js 15.
...
docker build -t luminous-ledger .
docker run -p 3000:3000 --env-file .env.local -v $(pwd)/data:/app/data luminous-ledger

# 4. Open in browser
# http://localhost:3000
```

> Note: If a `Dockerfile` does not exist yet in the `luminous-ledger/` directory, create one
...
5. Make sure Luminous Ledger is running on your computer (`npm run dev` or Docker)
...
### For persistent access, keep Luminous Ledger running
...
If no Dockerfile exists in `luminous-ledger/`, create this file:
...
# Restart the server
# (If using Docker, rebuild: docker build -t luminous-ledger . && docker run ...)
```
